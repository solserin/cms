<template>
  <div class="mt-4 pb-3">
    ...
    <password-checker accion="Actualizar funeraria" :show="showChecker" :callback-on-success="saveData" :callback-on-error="closeChecker" @closeVerificar="closeChecker"/>
  </div>
</template>
<script>
//use-utc	
...
export default {
    ...
    methods: {
        ...
        saveData() {
            ...
            let attributesUppercase = {
                funeraria: ['nombre_comercial', 'razon_social', 'rfc', 'calle', 'num_ext', 'num_int', 'colonia', 'telefono', 'ext', 'fax'],
            }
            ...
            self.closeChecker()
            self.$vs.loading()
            empresaService.updateFuneraria(self.funeraria).then((response) => {
    ...
    created: function () {
        ...
        satService.getRegimenes().then((response) => {
            if (response.status === 200) {
                self.regimenes = response.data.data.map(obj => { return { value: obj.id, label: obj.regimen } })
            }
        })
