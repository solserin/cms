  ...
  <div>
    <form>
        <vs-alert color="danger" title="Funeraria no encontrada" v-if="!funeraria">
            <span>Porfavor, registre la informacion de la funeraria antes de registrar informacion del cementerio</span>
        </vs-alert>
        <div class="mt-4 pb-3" v-if="funeraria">
            <div class="vx-row w-full">
                <div class="vx-col w-full">
                    <div class="flex items-end">
                        ...
                        <span class="leading-none font-medium">Informacion del cementerio</span>
                    </div>
                </div>
            </div>
            <vs-divider/>
            <div class="vx-row">
                <div class="vx-col w-full mt-3 md:w-3/5">
                    <label for="" class="vs-input--label">Nombre del cementerio <span class="text-danger text-sm">(*)</span>:</label>
                    <vs-input class="w-full uppercase" :disabled="disableFields" icon-pack="feather" icon="icon-user" data-vv-as="Nombre comercial" v-model="cementerio.cementerio" v-validate="'required'"  name="cementerio"/>
                    <span class="text-danger text-sm" v-show="errors.has('cementerio')">{{ errors.first('cementerio') }}</span>
                </div>
                <div class="vx-col w-full mt-3 md:w-1/5">
                    <label for="" class="vs-input--label">Telefonos <span class="text-danger text-sm">(*)</span>:</label>
                    <vs-input class="w-full uppercase" :disabled="disableFields" data-vv-as="Telefonos" icon-pack="feather" icon="icon-phone" v-model="cementerio.telefonos" v-validate="'required'" name="telefonos"/>
                    <span class="text-danger text-sm" v-show="errors.has('telefonos')">{{ errors.first('telefonos') }}</span>
                </div>
                <div class="vx-col w-full mt-3 md:w-1/5">
                    <vs-input class="w-full uppercase" :disabled="disableFields" data-vv-as="Fax" icon-pack="feather" icon="icon-file" v-model="cementerio.fax" label="Fax:" name="fax"/>
                </div>
            </div>
            ...
            <div class="vx-row">
                <div class="vx-col w-full md:w-3/12 mt-3">
                    ...
                    <vs-input :disabled="disableFields" name="calle" icon-pack="feather" icon="icon-map" data-vv-as="Calle" v-validate="'required'" v-model="cementerio.calle" class="w-full uppercase"/>
                ...
                <div class="vx-col w-full md:w-2/12 mt-3">
                    ...
                    <vs-input :disabled="disableFields" name="num_ext" icon-pack="feather" icon="icon-map" data-vv-as="Numero exterior" v-validate="'required'" v-model="cementerio.num_ext" class="w-full uppercase"/>
                ...
                <div class="vx-col w-full md:w-2/12 mt-3">
                    ...
                    <vs-input :disabled="disableFields" name="num_int" icon-pack="feather" icon="icon-map" data-vv-as="Numero interior" v-model="cementerio.num_int" class="w-full uppercase"/>
                ...
                <div class="vx-col w-full md:w-3/12 mt-3">
                    ...
                    <vs-input :disabled="disableFields" name="colonia" icon-pack="feather" icon="icon-map" data-vv-as="Colonia" v-validate="'required'" v-model="cementerio.colonia" class="w-full uppercase"/>
                ...
                <div class="vx-col w-full md:w-2/12 mt-3">
                    ...
                    <vs-input :disabled="disableFields" name="cp" icon-pack="feather" icon="icon-map" data-vv-as="C.P." v-validate="'required|numeric'" v-model="cementerio.cp" class="w-full uppercase"/>
            ...
            <div class="vx-row">
                <div class="vx-col w-full">
                    <div class="flex flex-wrap items-center justify-end">
                        <vs-button size="small" v-if="!disableFields" class="ml-auto mt-2" icon-pack="feather" icon="icon-save" @click.prevent="save">Guardar Cementerio</vs-button>
                    </div>
                </div>
            </div>
        </div>
    </form>
    <password-checker accion="Actualizar cementerio" :show="showChecker" :callback-on-success="saveData" :callback-on-error="closeChecker" @closeVerificar="closeChecker" />
  </div>
</template>
<script>
//use-utc	
...
export default {
    ...
    methods: {
        ...
        saveData() {
            let self = this
            self.cementerio.localidades_id = self.selectedLocalidad.value
            //uppercase attributes
            let attributesUppercase = ['cementerio', 'calle', 'num_ext', 'num_int', 'colonia'];

            _.forEach(attributesUppercase, (value) => {
                if (self.cementerio[value])
                    self.cementerio[value] = self.cementerio[value].toUpperCase()
            })
            self.closeChecker()
            self.$vs.loading()
            empresaService.updateCementerio(self.cementerio).then((response) => {
                self.$vs.loading.close()
        ...
        loadCementerio () {
            let self = this
            self.$vs.loading()
            empresaService.getCementerio().then((response) => {
                self.$vs.loading.close()
                let cementerio = response.data

                self.selectedEstado = {
                    value: cementerio.localidad.municipio.estado.id,
                    label: cementerio.localidad.municipio.estado.nombre
                }

                self.selectedMunicipio = {
                    value: cementerio.localidad.municipio.id,
                    label: cementerio.localidad.municipio.nombre
                }

                self.selectedLocalidad = {
                    value: cementerio.localidad.id,
                    label: cementerio.localidad.nombre
                }

                delete cementerio.localidad

                self.cementerio = _.clone(cementerio)
            }).catch((e) => {        
                self.$vs.loading.close()
            })
        }
