    ...
    <form>
      <div class="vx-row">
        <div class="vx-col w-full mt-3 flex justify-center">
          <div class="flex items-center">
            ...
            <div class="align-center">
              <vx-tooltip
                text="Archivo proporcionado por el SAT, es usado para poder realizar sellos en la facturación electronica, este debe corresponder al certificado de otra manera la facturación fallara"
              >
                <vs-upload
        ...
        <div class="vx-col w-full mt-3 flex justify-center">
          <div class="flex items-center">
            <vs-alert active="true" v-show="keyFilePath || cerFilePath">
              El archivo CER y KEY ya se encuentra registrados, si desea reemplazar cualquiera de estos porfavor suba el archivo correspodiente.
              <b>SI EL ARCHIVO QUE INTENTA SUBIR NO ES VALIDO, ESTE SERA IGNORADO Y SE MANTENDRA EL ARCHIVO QUE SE ENCUENTRA ACTUALMENTE REGISTRADO.</b>
            </vs-alert>
          </div>
        </div>
      </div>
      <div class="vx-row w-full">
        <div class="vx-col md:w-3/12 mt-3">
          <vx-tooltip
            text="La contraseña del certificado no se muestra, para cambiarla basta con introducirla en el campo de texto"
          >
            <vs-input
              v-model="facturacion.password"
              icon-pack="feather"
              icon="icon-lock"
        ...
        <div class="vx-col md:w-3/12 mt-3">
          <vs-input
            ...
            v-validate="rulesRepeat"
            name="repeat_password"
          />
          <span
            class="text-danger text-sm"
            v-show="errors.has('repeat_password')"
          >{{ errors.first('repeat_password') }}</span>
        </div>
        <div class="vx-col md:w-3/12 mt-3">
          <vx-tooltip text="Correo usado para emitir facturas">
            <vs-input
              ...
              icon="icon-mail"
              :bails="false"
              v-model="facturacion.email_emisor"
              class="w-full"
              label="Correo*:"
              type="email"
        ...
        <div class="vx-col w-full md:w-3/12 mt-3">
          ...
          <span
            class="text-danger text-sm"
            v-show="errors.has('moneda')"
          >{{ errors.first('moneda') }}</span>
        </div>
      </div>
      <vs-divider />
      <div class="vx-row">
        <div class="vx-col w-full">
          <div class="flex flex-wrap items-center justify-end">
            <vs-button
              ...
              icon="icon-save"
              @click.prevent="save"
            >Guardar Facturacion</vs-button>
          </div>
        </div>
      </div>
    </form>
    <password-checker
      accion="Actualizar facturacion"
      :show="showChecker"
...
export default {
  ...
  methods: {
    ...
    successCertificate() {
      let self = this;
      self.facturacion.certificateFile =
        self.$refs.upload.filesx[self.$refs.upload.filesx.length - 1];
    ...
    successKey() {
      let self = this;
      self.facturacion.keyFile =
        self.$refs.uploadKey.filesx[self.$refs.uploadKey.filesx.length - 1];
      self.keyError = false;
      self.$vs.notify({
        position: "top-center",
        color: "success",
    ...
    errorKey(event) {
      let self = this;
      self.$refs.uploadKey.srcs[
        self.$refs.uploadKey.srcs.length - 1
      ].error = true;
      self.facturacion.keyFile = null;
      let data = JSON.parse(event.target.response);
      self.$vs.notify({
